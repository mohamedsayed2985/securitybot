...
'message_id'=>$message_id2,
'text'=>"♔︙العضو ← [$namee](tg://user?id=$idcarlos)
♔︙تم تنزيله من ← المنشيئن الاساسين",
'parse_mode'=>'MarkDown',
'disable_web_page_preview'=>true,
'reply_to_message_id'=>$message_id,
"reply_markup"=>json_encode([
...
[['text'=>"♔ قناة السورس ♔",'url'=>"t.me/botatiiii"]],
]])
]); 
}
}
...
♔︙عزيزي ~⪼ $info
♔︙ شخص المطلوب ~⪼ $an
♔︙ قم بأرسال عدد النقاط ليتم اضافتهم
",
 'reply_to_message_id'=>$message_id
,]);}}}
...
'text'=>"
♔︙اهلأ عزيزي ← $info
♔︙نقاطك الحاليه ← .".$game['game'][$chat_id][$from_id].".
",
'parse_mode'=>"MARKDOWN",'disable_web_page_preview'=>true,'reply_to_message_id'=>$message->message_id,
]);
}
...
if($msgs['msgs'][$chat_id][$from_id] > 8000){
$active = array("مجنن القروب ",);
$abstfal = array_rand($active,1);
}elseif($msgs['msgs'][$chat_id][$from_id] > 7000){
$active = array("تفاعل حلو ",);
$abstfal = array_rand($active,1);
}elseif($msgs['msgs'][$chat_id][$from_id] > 6000){
$active = array("جهنم حبي ","نار وشرار ",);
$abstfal = array_rand($active,1);
}elseif($msgs['msgs'][$chat_id][$from_id] > 3000){
$active = array("احلى متفاعل ","اسطورة التفاعل ","يجنن تفاعلك كمل ","نايس التفاعل ","قمه التفاعل ",);
$abstfal = array_rand($active,1);
}elseif($msgs['msgs'][$chat_id][$from_id] > 500){
...
$like = array("يعني صوره مشي حالك ☹️❤️","شهل عسل ،₍🍯😻⁾","يهلا بلعافيه😍","مارتاحلك😐","تحبني؟🙈","فديت الحلو🌚😹","قمر والله🥺","موحلو والله☹️","ملاك وناسيك بقروبنا😟","لاصق فيها غيرها خلاص😒");
...
$a = str_replace('{النقاط}',$game['game'][$chat_id][$from_id],$a);
...
bot('sendMessage',[
'chat_id'=>$chat_id, 
'text'=>"اديلو وانا كيف اطيق خلقتك 😾🖖🏿🕷",
'reply_to_message_id'=>$message->message_id, 
]);
}}
if($text == "باي"){
...
bot('sendMessage',[
'chat_id'=>$chat_id, 
'text'=>"تطيب روحك وحياتك ي حياتي 💭♥️",
'reply_to_message_id'=>$message->message_id, 
]);
}}
if($text == "واو"){
...
bot('sendMessage',[
'chat_id'=>$chat_id, 
'text'=>"لو زاحفتلك كان ماقلت زاحفه 🌝🌸",
'reply_to_message_id'=>$message->message_id, 
]);
}}
if($text == "حفلش"){
...
elseif( $text =="تفعيل غنيلي" or $text == "تفعيل الاغاني"){
...
if ( $text =="تعطيل غنيلي" or $text == "تعطيل الاغاني"){
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$carlos) or in_array($from_id,$nazar) or in_array($from_id,$eri)) {$add = $settings["information"]["added"];
if ($add == true) {
	bot('sendmessage',[
	'chat_id'=>$chat_id,
